...
install_download() {
    ...
    [ -d ./MinerProxy ] && rm -rf ./MinerProxy-eth
...
start_write_config() {
    ...
    echo "下载完成，开启守护"
    echo
    chmod a+x $installPath/minerProxy.linux
    if [ -d "/etc/supervisor/conf/" ]; then
        rm /etc/supervisor/conf/MinerProxy.conf -f
        echo "[program:MinerProxy]" >>/etc/supervisor/conf/MinerProxy.conf
        echo "command=${installPath}/minerProxy.linux" >>/etc/supervisor/conf/MinerProxy.conf
        ...
        echo "command=${installPath}/minerProxy.linux" >>/etc/supervisor/conf.d/MinerProxy.conf
        ...
        echo "command=${installPath}/minerProxy.linux" >>/etc/supervisord.d/MinerProxy.ini
...
while :; do
    ...
    echo "github下载地址:https://github.com/mp-minerproxy/MinerProxy-eth"
    echo "官方TG群:https://t.me/MinerProxy_eth "
    echo
    echo " 1. 安装MinerProxy"
    echo
    echo " 2. 卸载MinerProxy"
